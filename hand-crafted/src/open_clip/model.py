...
class CLIP(nn.Module):
    ...
    def forward(self, image, text):
        ...
        text_features_no = self.encode_text(text[:,1,:], "no")
        text_features_no = F.normalize(text_features_no, dim=-1)
        '''
        num_prompt = text.size(1)
        text_f_no =[]
        for i in range(1, num_prompt):
            text_features_no = self.encode_text(text[:,i,:], "no")
            text_features_no = F.normalize(text_features_no, dim=-1)
            text_f_no.append(text_features_no.unsqueeze(1))
        if not self.training:
            return image_features, text_features, text_f_no[0][:,0,:], self.logit_scale.exp()
        text_f_no = torch.cat(text_f_no, dim=1)
        '''
