...
class ClipLoss(nn.Module):
    ...
    def image_text_binary_opposite_loss(self, logits_per_image_yes_no, eyes):
        ...
        ###### Old codes which are for CVPR 2022 submission.
        
        '''
        scale = 1.0
        logits_per_image_yes_no = torch.cat([logits_per_image.unsqueeze(-1) * scale, logits_per_image_no.unsqueeze(-1) * scale], dim=-1)
        logits_per_image_yes_no = F.softmax(logits_per_image_yes_no, dim=-1)
        
        
        K = int(num_logits * 0.1)
        loss_bin_yes, loss_bin_no = self.max_min_loss(K, all_image_features, all_text_features, logits_per_image_yes_no, eyes, num_logits, mode="all")
        
        
        total_loss_no = ((all_text_features * all_text_features_no).sum(-1) / 2 + 0.5).mean()
        return loss_bin_yes, loss_bin_no, total_loss_no

    def max_min_loss(self, K, all_image_features, all_text_features, logits_per_image_yes_no, eyes, num_logits, mode="max"):
        if mode != "all":   
            with torch.no_grad():
                inter_batch = all_image_features @ all_text_features.T  
                if mode == "max":     # biggest
                    inter_batch = inter_batch - 2 * eyes
                    _, idx = torch.sort(inter_batch, dim=1, descending=True)  
                    _, idx_T = torch.sort(inter_batch.T, dim=1, descending=True)
                else:
                    inter_batch = inter_batch + 2 * eyes
                    _, idx = torch.sort(inter_batch, dim=1, descending=False)  
                    _, idx_T = torch.sort(inter_batch.T, dim=1, descending=False)
                
                
            binary_no = torch.gather(logits_per_image_yes_no[:,:,1], dim=1, index=idx[:,:K])
            loss_bin_no = - torch.log(binary_no).mean()
            
            binary_no_T = torch.gather(logits_per_image_yes_no[:,:,1].T, dim=1, index=idx_T[:,:K])
            loss_bin_no_T = - torch.log(binary_no_T).mean()
            
            loss_bin_no = ( loss_bin_no + loss_bin_no_T  ) / 2
            
            binary_yes = torch.diagonal(logits_per_image_yes_no[:,:,0])
            loss_bin_yes = - torch.log(binary_yes).mean() 
        else:
            N = logits_per_image_yes_no.shape[0]
            binary_yes_no = eyes * logits_per_image_yes_no[:,:,0] + (1-eyes) * logits_per_image_yes_no[:,:,1]
            loss_bin = - torch.log(binary_yes_no) 
